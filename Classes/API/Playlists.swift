...
fileprivate func playlistContentChangeByApi(token: String, ownerId: String, playlistId: String, changeOperationsJson: String, revision: Int, completion: @escaping (_ result: Result<Playlist, YMError>) -> Void) {
    guard let req: URLRequest = buildRequest(for: .playlist_content_change(ownerId: ownerId, playlistId: playlistId, revision: revision, changeOperationsJson: changeOperationsJson, secret: token)) else {completion(.failure(.badRequest(errCode: -1, description: "Unable to build plylist content change request"))); return}
    requestYMResponse(req) { result in
        do {
            let ymResponse = try result.get()
            if let g_error = ymResponse.error {
